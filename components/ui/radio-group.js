...
export function RadioGroupItem({ className, ...props }) {
  return (
    <RadioGroupPrimitive.Item
        ...
        className={cn(
        "aspect-square size-[16px] shrink-0 cursor-pointer appearance-none rounded-full border border-[var(--fx-border)] bg-[var(--fx-surface)] text-[var(--fx-primary)] shadow-sm outline-none transition-colors hover:bg-[var(--fx-surface-hover)] focus-visible:ring-2 focus-visible:ring-[var(--fx-primary)]/20 disabled:cursor-not-allowed disabled:opacity-50 data-[state=checked]:border-[var(--fx-primary)]",
        className,
      )}
      {...props}
    >
