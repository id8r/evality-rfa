...
const DEMO_CANDIDATE_EXPERIENCE_POOL = [2.5, 4, 5, 7];
...
function inferCandidateExperience(candidate, seed) {
  if (candidate?.experience != null && candidate.experience !== "") {
    return Number(candidate.experience);
  }

  return DEMO_CANDIDATE_EXPERIENCE_POOL[seed % DEMO_CANDIDATE_EXPERIENCE_POOL.length] ?? null;
}
/* - - - - - - - - - - - - - - - - */

function inferCandidateMatchScore(candidate, seed) {
...
function normalizeStoredCandidate(candidate, job) {
  ...
  const targetJobId = candidate.jobId ?? job?.id ?? null;
  const existingJobContext = targetJobId ? jobContexts[targetJobId] ?? {} : {};
  const syntheticEmailStartedAt =
    candidate.status === "unscreened" && seed % 11 === 0
      ? new Date(Date.now() - ((seed % 2) + 1) * 24 * 60 * 60 * 1000).toISOString()
      : "";
  const normalizedJobContexts = targetJobId
    ? {
        ...jobContexts,
        [targetJobId]: {
          ...existingJobContext,
          emailScreeningStartedAt: existingJobContext.emailScreeningStartedAt ?? syntheticEmailStartedAt,
          emailScreeningAttemptCount:
            existingJobContext.emailScreeningAttemptCount != null
              ? Number(existingJobContext.emailScreeningAttemptCount)
              : syntheticEmailStartedAt
                ? 1 + (seed % 2)
                : 0,
        },
      }
  ...
  return {
    ...
    matchScore: inferCandidateMatchScore(candidate, seed),
    experience: inferCandidateExperience(candidate, seed),
    ...
    jobContexts: normalizedJobContexts,
  };
}
...
function createSyntheticCandidate(job, status, index, usedNames = new Set()) {
  ...
  const isFreshUnscreenedCandidate = status === "unscreened" && index < 2;
  ...
  let email = createCandidateEmail(name, job, index);
  let phone = createCandidatePhone(seed);

  if (status === "unscreened" && index === 0) {
    email = "";
  }

  if (status === "unscreened" && index === 1) {
    phone = "";
  }
  ...
  return {
    ...
    experience: DEMO_CANDIDATE_EXPERIENCE_POOL[seed % DEMO_CANDIDATE_EXPERIENCE_POOL.length],
    ...
    jobContexts: {
      [job.id]: {
        viewedAt: isFreshUnscreenedCandidate ? null : new Date(baseTimestamp - (hoursBack + 12) * 60 * 60 * 1000).toISOString(),
        screeningModeOverride: "ai",
        emailScreeningStartedAt: status === "unscreened" && index < 2 ? new Date(baseTimestamp - (index + 1) * 24 * 60 * 60 * 1000).toISOString() : "",
        emailScreeningAttemptCount: status === "unscreened" && index < 2 ? index + 1 : 0,
        screeningCompletedAt: status === "screened" && index < 2 ? new Date(baseTimestamp - (index + 2) * 24 * 60 * 60 * 1000).toISOString() : "",
      },
    },
    createdAt: new Date(baseTimestamp - Math.max(1, hoursBack - 1) * 60 * 60 * 1000).toISOString(),
