...
export function ensureDemoStore() {
  ...
  const storedJobs = ensureStoredCollection(STORAGE_KEYS.JOBS, DEMO_JOBS);
  const jobs = storedJobs.map((job, index) => decorateDemoJob(job, index));
  const storedCandidates = readStoredCollection(STORAGE_KEYS.CANDIDATES);
  const candidates = storedCandidates ? syncCandidatesWithJobs(jobs, storedCandidates) : syncCandidatesWithJobs(jobs, DEMO_CANDIDATES);

  if (JSON.stringify(jobs) !== JSON.stringify(storedJobs)) {
